...
public class ParallelMultiInstanceBehavior extends MultiInstanceActivityBehavior {
    ...
    public void leave(DelegateExecution execution) {
        ...
        if (executionEntity.getParent() != null) {
            ...
            boolean asyncLeave = usesParallelMultiInstanceLoopVariable && isAsyncLeave();
            if (!asyncLeave) {
            ...
            if (nrOfCompletedInstances >= nrOfInstances || isCompletionConditionSatisfied) {
                ...
                job.setElementId(flowElement.getId());
                job.setElementName(flowElement.getName());
                job.setJobHandlerType(ParallelMultiInstanceActivityCompletionJobHandler.TYPE);
                ...
                if (jobCategoryElements != null && jobCategoryElements.size() > 0) {
                    ...
                    if (StringUtils.isNotEmpty(jobCategoryElement.getElementText())) {
                        Expression categoryExpression = CommandContextUtil.getProcessEngineConfiguration().getExpressionManager()
